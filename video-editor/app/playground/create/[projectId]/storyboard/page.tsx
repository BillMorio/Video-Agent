...
export default function DraftStoryboardPage() {
  ...
  useEffect(() => {
    const projectAudio = localStorage.getItem(`project_${projectId}_audio_url`);
                        ...
                        <Card className="bg-card/40 border-border/40 rounded-2xl overflow-hidden shadow-sm">
                          <CardContent className="p-6 space-y-5">
                            {VISUAL_OPTIONS.map((opt) => (
                              <div key={opt.id} className="flex items-center space-x-3 group cursor-pointer" onClick={() => toggleVisualType(opt.id)}>
                                <Checkbox 
                                  id={opt.id} 
                                  checked={allowedVisualTypes.includes(opt.id)}
                                  onChange={() => toggleVisualType(opt.id)}
                                  className="w-5 h-5 rounded border-border/60"
                                />
                                <Label 
                                  htmlFor={opt.id} 
                                  className="text-[11px] font-black uppercase tracking-widest cursor-pointer group-hover:text-primary transition-colors italic"
                                >
                                  {opt.label}
                                </Label>
                              </div>
